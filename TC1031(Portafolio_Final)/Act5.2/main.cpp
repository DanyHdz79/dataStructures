...
vector<string> procesaIP(string ipIn) {
  ...
  for(int h=0; h<vector_ips.size()-1;h++){
    int intIp=stoi(vector_ips[h]);
    vector_ips_int.push_back(intIp);
  }
  vector_final.push_back(to_string(vector_ips_int[0])+"."+to_string(vector_ips_int[1])); //Primeros dos num de Ip (red)
  vector_final.push_back(to_string(vector_ips_int[2])+"."+to_string(vector_ips_int[3])); //Segundos dos num de Ip (host)
  vector_final.push_back(to_string(vector_ips_int[4])); //Último num de Ip (puerto)

  return vector_final;
}

//Complejidad O(n)
string procesaRed(string ipIn) {
  //Procesa el string de la IP y lo separa en componentes
  string delimiter1 = ".";
  int pos = 0;
  string token;
  ...
  for(int h=0; h<vector_ips.size();h++){
    int intIp=stoi(vector_ips[h]);
    vector_ips_int.push_back(intIp);
  }
  return (to_string(vector_ips_int[0])+"."+to_string(vector_ips_int[1])); //Primeros dos num de Ip (red)
...
Summary* search(Summary* hT[], string tmpKey) {
  string key = procesaRed(tmpKey);
